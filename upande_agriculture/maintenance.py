...
def check_current_journey_start_field():
    import frappe
    meta = frappe.get_meta("Bucket QR Code")
    print("has field:", any(df.fieldname == "current_journey_start" for df in meta.fields))


def force_abandon_journey(bucket_ids):
    """Test helper: simulates release_stale_buckets abandoning a journey
    (bucket left In Use from a prior day) without waiting a real day --
    releases back to Available, clears last_stock_entry and
    current_journey_start, but leaves the harvest entry itself unclaimed."""
    import frappe
    frappe.db.set_value(
        "Bucket QR Code", {"name": ["in", bucket_ids]},
        {"status": "Available", "last_stock_entry": None, "current_journey_start": None},
        update_modified=False,
    )
    frappe.db.commit()
    print("abandoned:", bucket_ids)


def cleanup_journey_test_data():
    import frappe

    buckets = [
        "TEST-JOURNEY-STD-01", "TEST-JOURNEY-STD-02", "TEST-JOURNEY-STD-03",
        "TEST-JOURNEY-SPRAY-01", "TEST-JOURNEY-CLEAN-01",
    ]
    names = frappe.get_all("Stock Entry", filters={"custom_bucket_id": ["in", buckets]}, pluck="name")

    # Clear cross-links + receiving links first (direct db access here, no
    # allow_on_submit REST restriction like on kaitet-group).
    frappe.db.set_value("Stock Entry", {"name": ["in", names]}, "custom_grading_entry", None, update_modified=False)
    frappe.db.set_value("Stock Entry", {"name": ["in", names]}, "custom_harvest_entry", None, update_modified=False)
    frappe.db.set_value("Stock Entry", {"name": ["in", names]}, "custom_receiving_entry", None, update_modified=False)
    frappe.db.set_value("Bucket QR Code", {"name": ["in", buckets]}, "last_stock_entry", None, update_modified=False)
    frappe.db.commit()

    docs = frappe.get_all("Stock Entry", filters={"name": ["in", names]}, fields=["name", "docstatus"])
    for d in docs:
        if d.docstatus == 1:
            frappe.get_doc("Stock Entry", d.name).cancel()
    for d in docs:
    ...
    for b in ["TEST-JRN-BUNCH-01", "TEST-JRN-BUNCH-02", "TEST-JRN-BUNCH-03",
              "TEST-JRN-BUNCH-04", "TEST-JRN-BUNCH-05", "TEST-JRN-BUNCH-06", "TEST-JRN-BUNCH-07"]:
        if frappe.db.exists("Bunch QR Code", b):
            frappe.delete_doc("Bunch QR Code", b, ignore_permissions=True, force=True)
    print("removed test bunch codes")
